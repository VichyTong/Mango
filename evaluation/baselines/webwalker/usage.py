...
# Wrapper function for programmatic usage
def webwalker_navigate(website: str, query: str, llm_config: dict, max_rounds: int = 100) -> str:
    tools = ["visit_page"]
    llm_config["query"] = query
    llm_config["action_count"] = max_rounds
    bot = WebWalker(llm=llm_config, function_list=tools)
...
def main():
    """Example usage of webwalker_navigate function"""
    import os
    ...
    if 'DASHSCOPE_API_KEY' in os.environ:
        ...
        raise ValueError("Please set DASHSCOPE_API_KEY or (OPENAI_API_KEY and OPENAI_MODEL_SERVER)")

    answer = webwalker_navigate(
        website="https://2025.aclweb.org/",
        query="When is the paper submission deadline for ACL 2025 Industry Track, and what is the venue specific address?",
...
if __name__ == "__main__":
    # Run the wrapper function, not the Streamlit app
    main()