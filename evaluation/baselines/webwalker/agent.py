...
class WebWalker(FnCallAgent):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        if isinstance(llm, dict) and llm.get('model', '').startswith('gpt-5-mini'):
            if 'generate_cfg' in llm and 'top_p' in llm['generate_cfg']:
                llm = llm.copy()
                llm['generate_cfg'] = llm['generate_cfg'].copy()
                llm['generate_cfg'].pop('top_p')
        super().__init__(function_list=function_list,
                         ...
                         **kwargs)
        self.llm_cfg = llm
        if not llm.get('model', '').startswith('gpt-5-mini'):
    ...
    def observation_information_extraction(self, query, observation):
        ...
        for attempt in range(max_retries):
            try:
                # for qwen 3 model
                if self.llm_cfg['model'].startswith('qwen3'):
