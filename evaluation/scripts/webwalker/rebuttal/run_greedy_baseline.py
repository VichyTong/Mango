...
from llm_web_scraper.selector.greedy import greedy_navigation
...
async def evaluate_one_task(
    ...
    try:
        ...
        result = await greedy_navigation(
            urls=urls,
...
async def main(
    ...
    normalized_model_name = normalize_model_name_for_path(model_name)
    base_dir = os.path.join(output_dir, "webwalker", "rebuttal", normalized_model_name, "greedy")
    ...
    for i, actual_idx in enumerate(remaining_indices):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"Task {actual_idx} ({i + 1}/{len(remaining_indices)})")
        logger.info(f"{'='*80}")
    ...
    total = len(results)
    if total > 0:
        logger.info(f"Greedy with {model_name}: {successful}/{total} ({successful/total*100:.1f}%)")
