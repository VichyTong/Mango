...
async def main(
    ...
    normalized_model_name = normalize_model_name_for_path(model_name)
    base_dir = os.path.join(output_dir, "webwalker", "rebuttal", normalized_model_name, "embedding")
    ...
    logger.info("Loading WebWalkerQA dataset")
    ...
    total = len(results)
    if total > 0:
        logger.info(f"Embedding scoring with {model_name}: {successful}/{total} ({successful/total*100:.1f}%)")
